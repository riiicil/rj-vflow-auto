...
- **Post-Release Hardening — Image Mode Submission, Multi-Tier Fallback & simulateClick**: [COMPLETE] (Resolved image generation prompt injection desynchronization, coordinate-aware PointerEvent click simulation, eliminated simulateClick double-click regression on toggle popovers, enabled ProseMirror Enter keydown fallback, and enforced exact model string matching to prevent Nano Banana 2 vs Nano Banana 2 Lite collision)
