...
- **Engine Hardening — Option C MAIN-World reCAPTCHA Enterprise Hook & UI Native Trigger**: [COMPLETE] (Permanently bypassed synthetic DOM button click dropping on 0-credit Nano Banana image models via a dedicated MAIN-world script `src/content/flow_bridge.js` that monkey-patches `window.grecaptcha.enterprise.execute`, pre-mints fresh reCAPTCHA Enterprise tokens in a clean microtask context, and dispatches native button triggers directly in the MAIN world to invoke Angular's native generation pipeline with full UI DOM synchronization; fixed video mode misclassification for `image-to-video` and `frames`; eliminated `TrustedHTML` CSP warnings; retained native DOM click pipeline with multi-tier fallback for Veo 3.1 video generation)
