...
- **Engine Hardening — Option C MAIN-World reCAPTCHA Enterprise & batchexecute RPC Bridge**: [COMPLETE] (Permanently bypassed synthetic DOM button click dropping on 0-credit Nano Banana image models via a dedicated MAIN-world script `src/content/flow_bridge.js` that mints fresh reCAPTCHA Enterprise tokens via `window.grecaptcha.enterprise.execute(SITE_KEY, { action: 'IMAGE_GENERATION' })`, reads `WIZ_global_data.SNlM0e` CSRF token, and executes native batchexecute RPC `ogiZ0b` directly over `fetch()`; integrated with isolated content script via dual-channel CustomEvent and postMessage client `src/services/FlowBridgeClient.js`; retained native DOM click pipeline with multi-tier fallback for Veo 3.1 video generation)
